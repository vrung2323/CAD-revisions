function [assembly_activity]=Assembly_activity_function(As_across_bins,...
  assembly, spM, BinSizes, lagChoice, act_count)
...
% 
%   (optional) lagChoice   -  The user can chose if to consider only 
%  the assembly activation (marked at 
...
for n=1:length(as)
   ...
   switch act_count
       case 'full'
            [activity]=DetectAssembly(spM ,as{n},tb);  
            %  only full assembly occurrences are considered
       case 'partial'
            [activity]=DetectAssembly_additive(spM ,as{n},tb);  
            % partial activations are also took into account
       case 'combined'
            [activity]=DetectAssembly_additive_weighted(spM ,as{n},tb);  
            % partial activations are also took into account but 
            % weighting differently full assembly contributions 
            % from partial assembly contributions.  
   end 
   ...
   switch lagChoice
       ...
       case 'duration'
           ...
           activity_lag = zeros(alag+1,numel(activity));
           activity_lag(1,:)=activity;
           for j=1:alag
               activity_lag(j+1,:) = [zeros(1,j) activity(1:end-j)];
...
% Maintain separate cells for Sbin >= i
...
for i=1:maxrate
    ...
    activity(i,:)=sum(Sbin_shifted,1);   % Some FRACTION of assembly units have >= i spikes
    activity(i,activity(i,:)==1)=0;      % If only 1 unit, set = 0  
end
    
activationtot=sum(activity,1);   
activationtot(isnan(activationtot))=0;
activationtot=activationtot/length(eelements);   % Divide by #elements so 
                                                 % in units of activations
                                                 % per assembly, rather
                                                 % than activations per cell
                                                 %
end
...
for i=1:maxrate
    ...
    activity(i,:)= activity(i,:).^5;       %Max value = (#elem)^5
    activity(i,find(activity(i,:)==1))=0;  %Don't count single cell activations
end
    
activationtot=sum(activity,1);   %Max value = maxrate*(#elem)^5
activationtot(isnan(activationtot))=0;
activationtot=activationtot/length(eelements)^5;   %This normalizes back to [0,maxrate]
end
