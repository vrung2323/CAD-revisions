...
% criteria := 'biggest'  keeps assemblies with biggest dimension 
%   (I discard all assemblies whose elements are all present in a bigger assembly)
...
%   than th are clustered). th can assume values inside the (0,1) interval 
%   (~0 -> no clustering; ~1 -> all in the same cluster). In each group 
%   only one assembly is kept according
